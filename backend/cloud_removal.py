...
# Sentinel-2 SCL (Scene Classification Layer) values that mean "not usable":
...
def temporal_composite(image_stack, scl_stack):
    ...
    """
    stack = np.stack(image_stack, axis=0)  # (T, H, W, 4)
    scl = np.stack(scl_stack, axis=0)  # (T, H, W)
    is_clear = ~np.isin(scl.astype(int), list(CLOUD_SCL_VALUES))  # (T, H, W)

    t, h, w, c = stack.shape
    composite = np.zeros((h, w, c), dtype=np.float32)
    any_clear = is_clear.any(axis=0)  # (H, W)

    # Pixels with >=1 clear observation: take the median of the clear ones
    clear_stack = np.where(is_clear[..., None], stack, np.nan)
    with np.errstate(invalid="ignore"):
        clear_median = np.nanmedian(clear_stack, axis=0)  # (H, W, C)
    composite[any_clear] = clear_median[any_clear]

    # Pixels that were NEVER clear in any of our dates: best-effort plain median
