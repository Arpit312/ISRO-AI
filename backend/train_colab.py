...
# Phenology: month -> 8-dim vector. Simple sinusoidal encoding + monsoon flag.
def month_to_phenology_vec(month):
    angle = 2 * np.pi * month / 12
    kharif = 1.0 if month in [6,7,8,9,10] else 0.0   # monsoon/kharif season
...
class SyntheticCloudDataset(Dataset):
    def __init__(self, root, size=256):
        # Automatically find the cloudy directory in case of nested unzipping
        ...
        if self.cloudy_dir is None:
            # Fallback if unzipped into /content directly without top folder
    ...
    def __getitem__(self, idx):
        ...
        # Bias month sampling toward monsoon since that correlates with more/thicker clouds in India
        month = random.choices(range(1, 13), weights=[1,1,1,1,1,2,3,3,3,2,1,1])[0]
        phenology = month_to_phenology_vec(month)

        # SAR channel: zero for now — training and inference must match until real SAR fetch is wired in
...
def train():
    ...
    # For local testing, change path to: "d:/ISRO_AI_Project/synthetic_data"
